...
forecasting--mode identification--parking optimization pipeline. Section~\r\ref{sec:results} evaluates performance and robustness, and
...
We evaluate the full suite of parking policies---static baselines, the interpretable Route~B zone policy, and our mode-aware dynamic strategy---in Section~\r\ref{sec:results} using a unified evaluator and identical call streams. We also report windowed performance for peak and transition periods (Table~\r\ref{tab:task3-windows}).
...
Table~\ref{tab:task1-routeb} reports that the AR(1)-corrected predictor improves one-step 5-minute demand forecasting
...
(ii) the fraction of ``long waits'' exceeding a configurable threshold (in our reported tables, a relative threshold equal to the lobby-baseline P95 within each evaluated subset). Because our evaluation is simulation-based, reported times are
...
Table~\ref{tab:task3-strategy} compares parking policies including two static baselines (last-stop, lobby-return), the interpretable Route~B zone policy, and our dynamic mode-aware policy.
