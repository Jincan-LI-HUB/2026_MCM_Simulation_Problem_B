...
\begin{itemize}[leftmargin=1.2em,itemsep=0.25em,topsep=0.25em]
  ...
  \item \textbf{Task 2 (Mode discovery):} Cluster 5-minute operational feature vectors (intensity, directionality, dispersion, inter-floor tendency, service pressure) with KMeans, then assign manager-friendly labels (e.g., \emph{Up-Peak}, \emph{Down-Peak}, \emph{Idle/Low}).
...
We adopt a gradient-boosted decision tree regressor (GBDT) due to its ability to model nonlinear interactions among mixed feature types, while remaining robust to noise and irregular demand spikes\cite{gbdt_chen}. The dataset is split \emph{chronologically} to avoid temporal leakage: the first portion is used for training and the remaining portion for testing. We report mean absolute error (MAE) and root mean squared error (RMSE) for interpretability and sensitivity to extreme deviations.
...
\label{eq:kmedian}
\end{equation}
Because floors lie on a 1D line and $|f-p|$ captures the dominant travel component, this objective directly targets reduced expected response distance (and thus waiting time), consistent with the weighted facility location approach for elevator group control\cite{zhang2025}.
...
We recommend reporting at least one stress test:
(i) demand scaling (e.g., $\times 1.2$, $\times 1.5$),
(ii) decision frequency (5 min vs 10 min),
(iii) parameter perturbations ($v$ and $\tau_d$ $\pm 20\%$).
These tests validate that relative strategy ranking is stable under uncertainty.
