...
oindent	extit{Note: In Table~ef{tab:task3-strategy}, AWT and the 95th-percentile waiting time (P95) are reported in simulation time units (seconds under our parameterization). ``Long wait'' is defined relative to a configurable threshold; we report it primarily to compare tail risk across strategies. Absolute values should be re-calibrated when building-specific kinematics are known\cite{cibse2020guided,barney2015elevatortraffic}.}

is reported in simulation time units (seconds under our parameterization). We emphasize relative improvements and ranking stability over absolute magnitudes.}
...
\input{outputs/tab/appendixF_stress.tex}
