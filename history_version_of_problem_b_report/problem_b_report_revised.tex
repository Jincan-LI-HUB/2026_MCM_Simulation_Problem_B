...
\subsection{Key Assumptions and Notation Explaination}
...
\input{outputs/tab/task1_metrics.tex}
...
\input{outputs/tab/task2_cluster_profile.tex}
...
We report (i) average waiting time (AWT), defined as the time from a hall call until the first elevator arrival, and (ii) the fraction of ``long waits'' exceeding a threshold (e.g., 60 seconds). These metrics are standard, interpretable, and tightly coupled to the parking decision.

\input{outputs/tab/task3_strategy_comparison.tex}
...
\section{Result Analysis and Robustness Testing}
\label{sec:results}
